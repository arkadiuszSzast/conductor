...
a question text payload (typically carrying a `conductor-questions`
block) instead of an outcome. An `ask` SHALL NOT conclude the run or the
step: the run stays the step's active run and its session is preserved.
The feature SHALL enter `waiting_human` and the question SHALL be
persisted with the run so it survives a daemon restart. An `ask` against
a run that is not running SHALL be rejected the same way a stale report
is.

#### Scenario: Ask parks the feature without concluding the run

- **WHEN** an agent run reports `ask` with a question
