...
- [x] 1.1 [db] Migration: `pending_question TEXT`, `asked_at INTEGER` on
      `run`; surface both on `RunSummary`
- [x] 1.2 [server] Store methods: `setRunQuestion(runId, question)` /
...
- [x] 1.3 [test] Store tests: set/clear round-trip, feature status flips,
      transition-log entries, stale-run rejection, restart visibility

## 2. Engine: ask, answer, reconcile

- [x] 2.1 [server] `Engine.report` accepts the `ask` shape: persists the
      question via `setRunQuestion`, notifies (`notify?`), does NOT
      conclude the run
- [x] 2.2 [server] `Engine.answer(runId, notes)`: validate pending
...
- [x] 2.3 [server] `reconcileAgentRun`: skip nudge/idle-reap while
      `pending_question` is set; TTL reap unchanged; restart recovery
      leaves asking runs alone
- [x] 2.4 [test] Engine tests: ask parks feature + preserves run/session;
...
- [x] 3.1 [server] `POST /v1/runs/:id/report` accepts exclusive
      `{ ask }`; `POST /v1/runs/:id/answer` `{ notes }` with
      `no_pending_question` / `session_lost` conflict codes
- [x] 3.2 [server] Projections: `pendingQuestion`/`askedAt` on run
      summaries and the feature detail's active run
- [x] 3.3 [test] API tests: ask via report, answer happy path, conflict
      codes, question visible in detail, auth on the new endpoint

## 4. Surfaces: plugin, web, CLI

- [x] 4.1 [runner] `conductor_ask` tool in the opencode plugin ("only when
      a human decision is required to proceed"; payload documents the
      `conductor-questions` block convention)
- [x] 4.2 [web] Gate panel: when the waiting surface is an asking run,
      render the question through the existing parse/compose form and
      submit to the answer endpoint (approve/reject hidden)
- [x] 4.3 [cli] `conductor answer <run-id> --notes <text|->`; pending
      question printed by `status <feature-id>`
- [x] 4.4 [test] Web panel test (asking-run form, submit path); CLI answer
      + status tests

## 5. Docs

- [x] 5.1 [docs] Runner protocol: the ask report and answer flow;
