...
export function assembleDaemonConfig(raw: unknown): DaemonFileConfig {
  ...
  if (!Array.isArray(projects) || projects.length === 0 || !projects.every(isNonEmptyString)) {
    throw new UsageError('daemon config requires "projects" (non-empty array of project directory strings)')
  }
