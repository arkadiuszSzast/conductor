/**
 * Guards the pipeline engine extraction constraints:
...
describe("pipeline engine extraction constraints", () => {
  ...
  it("builtins never call a global runShell — only the injected ProcessRunner", () => {
    const source = codeOnly(readFileSync(join(ENGINE_DIR, "builtins.ts"), "utf-8"))
