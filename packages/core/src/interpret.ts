 ...
 * pure makes every pipeline shape unit-testable without a database, a
 * git repo, or an LLM.
 */
...
export function interpret(
  ...
  switch (event.kind) {
    case "feature.start":
      return enter(def, state, firstRunnableStep(def, state))
    case "step.succeeded":
    ...
    case "human.approved":
      return onHumanApproved(def, state, event.stepId)
    case "human.rejected":
...
function isSkipped(state: FeatureState, step: StepDef): boolean {
  // Feature-level skips are resolved into state.attempts?? No — skips are
  // static per feature: a skipped optional step is simply never entered.
  // The engine materialises config skipSteps into the feature's skip set;
  // here we only honour `optional` steps listed in state via attempts
  // sentinel. Simpler: the engine pre-filters — interpreter treats every
  // step in `def` as live. Kept as a hook for future per-feature skips.
  void state
  void step
  return false
}

function firstRunnableStep(def: PipelineDef, state: FeatureState): string | null {
  for (const step of def.pipeline) {
    if (!isSkipped(state, step)) return step.id
  }
  return null
}
...
function onHumanApproved(_def: PipelineDef, state: FeatureState, stepId: string): Transition {
